...
char *gen_makefile(toml_parsed_t *parsed, int count, const char *cmate_version, int output) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        project_names = format_string("%s%s%s", tmp ? tmp : "", (tmp ? " " : ""), parsed[i].project_name);
        free(tmp);
        if (!project_names) {
    ...
    char *project_names_line = format_string("PROJECT_NAMES := %s", project_names);
    free(project_names);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        char *cflags = format_string("CFLAGS_%s := %s", p->project_name, p->cflags ? p->cflags : "");
        if (!cflags) {
            ERR("Memory allocation failed for cflags\n");
            free(cc); free(all_vars); free(project_names_line);
            return NULL;
        }
        ...
        if (!ldlibs) {
            ...
            free(cc); free(cflags); free(all_vars); free(project_names_line);
            return NULL;
        }
        ...
        if (!ldflags) {
            ...
            free(cc); free(cflags); free(ldlibs); free(all_vars); free(project_names_line);
            return NULL;
        }
        ...
        if (!srcdirs) {
            ...
            free(cc); free(cflags); free(ldlibs); free(ldflags); free(all_vars); free(project_names_line);
            return NULL;
        }
        ...
        if (!includes) {
            ...
            free(cc); free(cflags); free(ldlibs); free(ldflags); free(srcdirs); free(all_vars); free(project_names_line);
            return NULL;
        }
        ...
        if (!compile_files) {
            ...
            free(cc); free(cflags); free(ldlibs); free(ldflags); free(srcdirs); free(includes); free(all_vars); free(project_names_line);
            return NULL;
        }

        char *tmp = all_vars;
        all_vars = format_string("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n",
                                 tmp, cc, cflags, ldlibs, ldflags, srcdirs, includes, compile_files);
        free(tmp); free(cc); free(cflags); free(ldlibs); free(ldflags);
        free(srcdirs); free(includes); free(compile_files);
    ...
    if (!copy) {
        ...
        free(all_vars); free(project_names_line);
        return NULL;
    }
    ...
    if (!first) {
        ...
        free(all_vars); free(project_names_line); free(copy);
        return NULL;
    }
    
    char *result = format_string("%s\n%s\n%s\n%s", first, project_names_line, all_vars, copy);
    if (!result) {
        ERR("Memory allocation failed for final result\n");
        free(all_vars); free(project_names_line); free(copy); free(first);
        return NULL;
    }

    if (output) printf("Makefile generated\n");

    free(project_names_line); free(all_vars); free(copy); free(first);
    return result;
}
